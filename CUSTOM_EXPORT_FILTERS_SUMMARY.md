...
## Изменения в версии 2.2 - Drag & Drop упорядочивание полей

- **Добавлена возможность настройки порядка полей** через drag & drop интерфейс
- **Новый блок "Настройте порядок полей"** появляется после выбора полей
- **Интуитивный интерфейс перетаскивания** с визуальными индикаторами
- **Нумерация позиций** - каждое поле показывает свой номер в экспорте
- **Быстрое удаление полей** - кнопка × для удаления поля из списка
- **Сохранение порядка в шаблонах** - порядок сохраняется и восстанавливается
- **Автоматическое скрытие/показ блока** в зависимости от выбранных полей

### Как использовать новую функциональность:

1. **Выберите источник данных** (полисы или платежи)
2. **Отметьте нужные поля** - появится блок упорядочивания
3. **Перетащите поля** для изменения их порядка в экспорте
4. **Используйте номера позиций** для контроля порядка
5. **Удаляйте ненужные поля** кнопкой ×
6. **Сохраните как шаблон** для повторного использования

Кастомный экспорт теперь предоставляет полный контроль над структурой отчетов!
