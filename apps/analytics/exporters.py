...
class AnalyticsExporter:
    ...
    def export_insurance_type_analytics(
        self,
        analytics: Dict[str, Any],
        applied_filters: Optional[Dict[str, Any]] = None,
    ) -> HttpResponse:
        ...
        Args:
            analytics: Insurance type analytics data
            applied_filters: Applied filters information
        ...
        try:
            workbook = Workbook()
            worksheet = workbook.active
            worksheet.title = "Insurance Type Analytics"

            # Add title and metadata
            worksheet.cell(row=1, column=1, value="Insurance Type Analytics Report")
            worksheet.cell(row=1, column=1).font = Font(size=16, bold=True)
            ...
            # Data rows
            insurance_type_metrics = analytics.get("insurance_type_metrics", [])
            start_data_row = current_row

            for type_metric in insurance_type_metrics:
                type_info = type_metric.get("insurance_type", {})
                worksheet.cell(
                    row=current_row, column=1, value=type_info.get("name", "")
                )
                worksheet.cell(
                    row=current_row,
                    column=2,
                    value=self._format_value(type_metric.get("premium_volume", 0)),
                )
                worksheet.cell(
                    row=current_row,
                    column=3,
                    value=self._format_value(type_metric.get("commission_revenue", 0)),
                )
                worksheet.cell(
                    row=current_row,
                    column=4,
                    value=self._format_value(type_metric.get("policy_count", 0)),
                )
                worksheet.cell(
                    row=current_row,
                    column=5,
                    value=self._format_value(type_metric.get("insurance_sum", 0)),
                )
                worksheet.cell(
                    row=current_row,
                    column=6,
                    value=self._format_value(
                        type_metric.get("average_commission_per_policy", 0)
                    ),
                )

                # Calculate market share if not provided
                market_share = type_metric.get("market_share", 0)
                worksheet.cell(
                    row=current_row, column=7, value=self._format_value(market_share)
                )
                current_row += 1
            ...
            response[
                "Content-Disposition"
            ] = f'attachment; filename="insurance_type_analytics_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx"'
