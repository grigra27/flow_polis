...
class AnalyticsService:
    ...
    def get_insurance_type_analytics(
        self, analytics_filter: Optional[AnalyticsFilter] = None
    ) -> Dict[str, Any]:
        """
        Get analytics data grouped by insurance type.

        Args:
            analytics_filter: Optional filter to apply to the data

        Returns:
            Dictionary containing insurance type analytics
        """
        try:
            from apps.policies.models import Policy, PaymentSchedule, InsuranceType
            from django.db.models import Count
            ...
            # Get insurance types that have policies
            insurance_types_with_data = InsuranceType.objects.filter(
            ...
            for insurance_type in insurance_types_with_data:
                # Filter data for this insurance type
                type_policies = policies_qs.filter(insurance_type=insurance_type)
                type_payments = payments_qs.filter(
                    policy__insurance_type=insurance_type
                )

                # Calculate metrics for this insurance type
                premium_volume = self.calculator.calculate_premium_volume(
                    type_payments, date_range
                )
                commission_revenue = self.calculator.calculate_commission_revenue(
                    type_payments, date_range
                )
                policy_count = self.calculator.calculate_policy_count(
                    type_policies, date_range
                )
                insurance_sum = self.calculator.calculate_insurance_sum(
                    type_payments, date_range
                )
                ...
                if policy_count > 0:
                    average_commission_per_policy = commission_revenue / Decimal(
                        str(policy_count)
                    )
                else:
                    average_commission_per_policy = Decimal("0")

                # Get branch distribution for this insurance type
                branch_distribution = dict(
                    type_policies.values("branch__branch_name")
                    .annotate(count=Count("id"))
                    .values_list("branch__branch_name", "count")
                )

                # Get insurer distribution for this insurance type
                insurer_distribution = dict(
                    type_policies.values("insurer__insurer_name")
                    .annotate(count=Count("id"))
                    .values_list("insurer__insurer_name", "count")
                )

                insurance_type_metrics.append(
                    {
                        "insurance_type": {
                        ...
                        "policy_count": policy_count,
                        "insurance_sum": insurance_sum,
                        "average_commission_per_policy": average_commission_per_policy,
            ...
            if insurance_type_metrics:
                most_profitable_type = max(
                    insurance_type_metrics, key=lambda x: x["commission_revenue"]
                )

            return {
                "insurance_type_metrics": insurance_type_metrics,
                "total_insurance_types": len(insurance_type_metrics),
            ...
            logger = logging.getLogger(__name__)
            logger.error(f"Error calculating insurance type analytics: {e}")
