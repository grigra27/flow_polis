...
def migrate_leasing_managers_forward(apps, schema_editor):
    ...
    default_manager, _ = LeasingManager.objects.get_or_create(
        name="Не указан",
        defaults={
            "notes": "Менеджер по умолчанию для полисов без указанного менеджера"
        },
    )

    # Получаем все уникальные значения менеджеров из существующих полисов
    unique_managers = (
        Policy.objects.exclude(leasing_manager="")
        .values_list("leasing_manager", flat=True)
        .distinct()
    )
    ...
    for manager_name in unique_managers:
        if manager_name and manager_name.strip():
            manager, _ = LeasingManager.objects.get_or_create(name=manager_name.strip())
    ...
    for policy in Policy.objects.all():
        if policy.leasing_manager and policy.leasing_manager.strip():
            policy.leasing_manager_new = manager_mapping.get(
                policy.leasing_manager, default_manager
            )
        else:
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(
            migrate_leasing_managers_forward, migrate_leasing_managers_backward
        ),
