...
class TestCopyPolicyAction:
    ...
    def test_copy_multiple_policies(self, policy_factory, client_factory, insurer_factory, insurance_type_factory, branch_factory):
        """
        ...
        queryset = Policy.objects.filter(
            id__in=[policy1.id, policy2.id, policy3.id]
        )
