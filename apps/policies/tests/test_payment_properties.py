...
class TestPaymentScheduleProperties(TestCase):
    ...
    @given(
        insurance_sum=st.decimals(
            min_value=Decimal("0.01"), max_value=Decimal("999999999999.99"), places=2
        )
    )
    def test_insurance_sum_round_trip(self, insurance_sum):
        ...
        **Validates: Requirements 1.1, 1.2**
        ...
        client = Client.objects.create(
            client_name="Test Client", client_inn="1234567890"
        )
        insurer = Insurer.objects.create(
            insurer_name="Test Insurer", contacts="test@example.com"
        )
        branch = Branch.objects.create(branch_name="Test Branch")
        insurance_type = InsuranceType.objects.create(name="Test Type")
        ...
        assert (
            retrieved_payment.insurance_sum == insurance_sum
    ...
    @given(
        ...
        min_range=st.decimals(
            min_value=Decimal("0.01"), max_value=Decimal("500000000.00"), places=2
        ),
    ...
    def test_insurance_sum_range_filtering(self, insurance_sums, min_range, max_range):
        ...
        **Validates: Requirements 4.4**
        ...
        # Create required related objects
        client = Client.objects.create(
            client_name="Test Client Filter", client_inn="1234567891"
        )
        insurer = Insurer.objects.create(
            insurer_name="Test Insurer Filter", contacts="test@example.com"
        )
        branch = Branch.objects.create(branch_name="Test Branch Filter")
        insurance_type = InsuranceType.objects.create(name="Test Type Filter")
        ...
        filtered_payments = PaymentSchedule.objects.filter(
            policy=policy, insurance_sum__gte=min_range, insurance_sum__lte=max_range
        )
        ...
        expected_in_range = [
            p for p in created_payments if min_range <= p.insurance_sum <= max_range
        ]
    ...
    @given(
        ...
        amount=st.decimals(
            min_value=Decimal("0.01"), max_value=Decimal("999999999.99"), places=2
        ),
        insurance_sum=st.decimals(
            min_value=Decimal("0.01"), max_value=Decimal("999999999999.99"), places=2
        ),
        kv_rub=st.decimals(
            min_value=Decimal("0.00"), max_value=Decimal("999999999.99"), places=2
        ),
        payment_info=st.text(
            max_size=200, alphabet=st.characters(blacklist_characters="\x00")
        ),
    )
    def test_payment_copy_field_identity(
        self,
        year_number,
        installment_number,
        amount,
        ...
        **Validates: Requirements 2.2**
        ...
        # Create required related objects
        client = Client.objects.create(
            client_name="Test Client Copy", client_inn="1234567892"
        )
        insurer = Insurer.objects.create(
            insurer_name="Test Insurer Copy", contacts="test@example.com"
        )
        branch = Branch.objects.create(branch_name="Test Branch Copy")
        insurance_type = InsuranceType.objects.create(name="Test Type Copy")
        commission_rate = CommissionRate.objects.create(
            insurer=insurer, insurance_type=insurance_type, kv_percent=Decimal("10.00")
        )
        ...
        insurer_date = (
            date.today() + timedelta(days=20) if year_number % 3 == 0 else None
        )
    ...
    def test_kv_percent_actual_calculation(self, amount, kv_rub):
        ...
        **Validates: Requirements 5.1, 5.2**
        ...
        # Create required related objects
        client = Client.objects.create(
            client_name="Test Client KV", client_inn="1234567893"
        )
        insurer = Insurer.objects.create(
            insurer_name="Test Insurer KV", contacts="test@example.com"
        )
        branch = Branch.objects.create(branch_name="Test Branch KV")
        insurance_type = InsuranceType.objects.create(name="Test Type KV")
    ...
    def test_kv_percent_actual_zero_amount(self):
        ...
        **Validates: Requirements 5.3**
        ...
        # Create required related objects
        client = Client.objects.create(
            client_name="Test Client Zero", client_inn="1234567894"
        )
        insurer = Insurer.objects.create(
            insurer_name="Test Insurer Zero", contacts="test@example.com"
        )
        branch = Branch.objects.create(branch_name="Test Branch Zero")
        insurance_type = InsuranceType.objects.create(name="Test Type Zero")
