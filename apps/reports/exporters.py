...
class ThursdayReportExporter(BaseExporter):
    ...
    def _add_section_header(self, ws, title, num_columns):
        ...
        ws.merge_cells(start_row=current_row, start_column=1, 
                      end_row=current_row, end_column=num_columns)
    ...
    def apply_formatting(self, ws):
                ...
                for cell in column_cells:
                    ...
                    if hasattr(cell, 'value') and cell.value:
                        ...
                        if isinstance(cell.value, str) and (
                            'ПЕРЕЧЕНЬ ДФА' in cell.value or 
                            'ПОЛИСЫ БЕЗ ДОКУМЕНТОВ' in cell.value or 
                            'НЕТ ДАННЫХ ОБ ОПЛАТЕ' in cell.value
                        ):
                            continue
        ...
        for row in ws.iter_rows(min_row=4):  # Пропускаем заголовок отчета, пустую строку и заголовки столбцов
            for idx, cell in enumerate(row, start=1):
                if cell.value and not isinstance(cell.value, str) or (isinstance(cell.value, str) and cell.value.replace('.', '').replace(',', '').isdigit()):
                    # Пропускаем заголовки разделов
                    if isinstance(cell.value, str) and ('ПОЛИСЫ БЕЗ ДОКУМЕНТОВ' in cell.value or 'НЕТ ДАННЫХ ОБ ОПЛАТЕ' in cell.value):
                        continue
        ...
        for row in range(5, ws.max_row + 1):
            ...
            if cell_value and isinstance(cell_value, str) and ('ПОЛИСЫ БЕЗ ДОКУМЕНТОВ' in cell_value or 'НЕТ ДАННЫХ ОБ ОПЛАТЕ' in cell_value):
                continue
