...
class LargeDatasetTest(TestCase):
    ...
    def setUpClass(cls):
        ...
        for i in range(10):
            client = Client.objects.create(
                client_name=f"Клиент {i}", client_inn=f"{1000000000 + i}"
            )
            cls.clients.append(client)

        cls.insurers = []
        for i in range(5):
            insurer = Insurer.objects.create(insurer_name=f"Страховщик {i}")
            cls.insurers.append(insurer)

        cls.branches = []
        for i in range(3):
            branch = Branch.objects.create(branch_name=f"Филиал {i}")
            cls.branches.append(branch)

        cls.insurance_types = []
        for i in range(4):
            ins_type = InsuranceType.objects.create(name=f"Вид страхования {i}")
    ...
    def test_export_1000_policies_performance(self):
        ...
        self.assertLess(
            elapsed_time, 10.0, f"Export took {elapsed_time:.2f}s, expected < 10s"
        )
    ...
    def test_export_with_filters_performance(self):
        ...
        filterset = PolicyExportFilter({"policy_active": True}, queryset=queryset)
...
class ConcurrentExportTest(TestCase):
    ...
    def setUp(self):
        ...
        self.client_obj = Client.objects.create(
            client_name="Клиент для последовательных тестов", client_inn="9999999999"
        )
    ...
    def test_sequential_exports(self):
        ...
        for i in range(num_exports):
            ...
            print(
                f"  Экспорт {i+1}: {results[-1]['time']:.2f}s, {results[-1]['size']/1024:.2f}KB"
            )
        ...
        self.assertLess(
            avg_time, 1.0, f"Average export time {avg_time:.2f}s is too high"
        )
...
class MemoryUsageTest(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.client = Client.objects.create(
            client_name="Клиент для теста памяти", client_inn="8888888888"
        )

        cls.insurer = Insurer.objects.create(insurer_name="СК для теста памяти")

        cls.branch = Branch.objects.create(branch_name="Филиал для теста памяти")

        cls.insurance_type = InsuranceType.objects.create(name="Тип для теста памяти")
