...
class PropertyTest1_ColumnCount(TestCase):
    ...
    @given(field_count=st.integers(min_value=1, max_value=10))
...
class PropertyTest2_FilterCorrectness(TestCase):
    ...
    def setUp(self):
        ...
        self.client1 = Client.objects.create(
            client_name="Alpha Corp", client_inn="1111111111"
        )
        self.client2 = Client.objects.create(
            client_name="Beta LLC", client_inn="2222222222"
        )
    ...
    def test_filtered_results_match_criteria(
        self, filter_active, use_branch_filter, use_insurer_filter
    ):
...
class PropertyTest3_TemplateRoundTrip(TestCase):
    ...
    def test_template_round_trip(self, data_source, field_count, filter_count):
        ...
        config = {"fields": fields, "filters": filters}
...
class PropertyTest4_DateFormatting(TestCase):
    ...
    @given(
        ...
        ),  # Используем 28 чтобы избежать проблем с февралем
    )
    def test_date_format_is_correct(self, year, month, day):
        ...
        exporter = CustomExporter(
            Policy.objects.filter(id=policy.id), ["start_date", "end_date"], "policies"
        )
...
class PropertyTest5_DecimalFormatting(TestCase):
    ...
    @given(
        premium=st.decimals(
            min_value=Decimal("1000.00"), max_value=Decimal("10000000.00"), places=2
        )
    )
    def test_decimal_converted_to_float(self, premium):
        ...
        self.assertAlmostEqual(premium_value, float(premium), places=2)
...
class PropertyTest6_QueryOptimization(TestCase):
    ...
    @given(use_related_fields=st.booleans())
