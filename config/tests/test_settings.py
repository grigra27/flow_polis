...
class SettingsEnvironmentVariablesTest(unittest.TestCase):
    ...
    **Property 2: Конфигурация через переменные окружения**
    ...
    def test_secret_key_from_environment(self):
        ...
        with patch.dict(os.environ, {'SECRET_KEY': test_secret, 'DEBUG': 'True'}, clear=True):
    ...
    def test_debug_from_environment(self):
        ...
        with patch.dict(os.environ, {'DEBUG': 'False', 'SECRET_KEY': 'test-key', 'DB_NAME': 'test', 'DB_USER': 'test', 'DB_PASSWORD': 'test', 'ALLOWED_HOSTS': 'test.com'}, clear=True):
            self._reload_settings()
            from config import settings
            self.assertFalse(settings.DEBUG)
        
        with patch.dict(os.environ, {'DEBUG': 'True', 'SECRET_KEY': 'test-key'}, clear=True):
            self._reload_settings()
            from config import settings
    ...
    def test_allowed_hosts_from_environment(self):
        ...
        with patch.dict(os.environ, {'ALLOWED_HOSTS': test_hosts, 'DEBUG': 'True', 'SECRET_KEY': 'test'}, clear=True):
            self._reload_settings()
            from config import settings
            self.assertEqual(settings.ALLOWED_HOSTS, ['example.com', 'www.example.com', 'api.example.com'])
    ...
    def test_celery_from_environment(self):
        ...
        with patch.dict(os.environ, celery_config, clear=True):
            ...
            self.assertEqual(settings.CELERY_RESULT_BACKEND, 'redis://test-redis:6379/2')
