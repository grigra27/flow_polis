...
### 1. **Threading в TelegramHandler** = unbounded fan-out

`emit()` создаёт `Thread(daemon=True)` на каждое логированное событие. Если приложение начнёт спамить ERROR в горячем цикле (например, миграция упала, и каждый http-запрос даёт ошибку) — будут сотни тредов одновременно. Защита только от rate-limit'а в `_should_send_message`, который проверяется **до** треда. Но создание треда на каждый chacked-out event с rate-limit reject = всё равно overhead.
...
### 5. **VK random_id уникален «в рамках одного запуска»**
