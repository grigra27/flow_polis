...
- **Проблема:** о проблемах никто не узнаёт: скрипты до P0-07/08 всегда завершались успехом, а health-check шлёт только в мёртвый Telegram (подтверждено на проде 2026-09-23: cron-строка передаёт `--notify-telegram` без `--notify-vk`).
...
### P1-10 — Сделать VK единственным практическим каналом уведомлений везде (решение Q2 реализуется)
...
- **Статус:** pending, переформулирована 2026-09-23 под решение Q2 (VK — единственный канал); конкретный пробел (флаг в cron health-check) уже подтверждён на проде и готов к правке.
...
- **P1-10 первой в фазе C.** Самый дешёвый и самый срочный пункт (флаг `--notify-vk` в cron health-check уже отсутствует на проде — подтверждено 2026-09-23) и прямая зависимость P1-08: мониторинг без рабочего канала алертов фиксировал бы проблемы молча — ровно тот дефект, ради которого он заводится.
...
| P1 | P1-10 | VK — единственный практический канал уведомлений | pending |
...
Итого 25 задач: 9 × P0 (все PASS), 10 × P1 (4 CANCELLED, 1 PASS, 5 pending), 6 × P2 (все pending). Дубликатов ID нет, нумерация в каждой группе непрерывна (P0-01…P0-09, P1-01…P1-10, P2-01…P2-06). Отменённые задачи сохраняют номер в реестре — не переиспользуется и не удаляется, чтобы решение оставалось прослеживаемым.
...
| P1-05, P1-06, P1-08, P1-10 | pending, объём пересмотрен под решение владельца (см. рев. 4) |
| P2-01…P2-06 | pending |
...
- Мониторинг и hardening доставки уведомлений (Telegram мёртв, VK — единственный живой канал; finding приоритета env-переменных в `telegram-config.sh`, зафиксированный в P0-09) → **P1-10** (Q2 закрыт: VK — единственный практический канал; уже найден конкретный пробел — cron health-check шлёт только `--notify-telegram`) / диагностика — **P1-07 (PASS)**.
