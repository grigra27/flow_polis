...
|---|---|---|
| `/root/insurance_broker` **не является git-чекоутом** (`.git` отсутствует), но `scripts/backup-db-telegram.sh`, `backup-media-telegram.sh`, `telegram-notify.sh` **байт-в-байт совпадают с репозиторием** (md5 `6972fbe…`, `1a24c0e…`, `d57946c…`) | сервер + repo | Репозиторий — источник истины, но нужен явный канал доставки правок на сервер → P0-03 |
...
### P0-03 — Зафиксировать воспроизводимый канал доставки скриптов бэкапа на сервер
- **Проблема:** сервер — не git-чекоут; правки репозитория физически не могут попасть в cron. Сейчас копии совпадают с repo, но это ничем не гарантировано.
- **Почему P0:** блокирует P0-04…P0-08; без него «исправлено в репозитории» ≠ «исправлено в проде».
- **Тронутые объекты:** `scripts/` на сервере; `docs/BACKUP_RESTORE.md`; `scripts/deploy.sh` (он пересобирает контейнеры, но файлы не синхронизирует).
- **Scope:** реализовать и задокументировать доставку по решению **D1** (`scp` отдельного файла + сверка md5 с repo + timestamped `.bak` на сервере перед заменой): копия → доставка → `md5sum` сравнение → smoke-run (`--help`). Сама задача не меняет рабочий код.
- **Не входит:** правка `deploy.sh` (пересборка контейнеров = downtime), cron, бэкап-скриптов, внедрение ansible.
- **Зависимости:** нет (решение принято инженерно, D1).
- **Риск:** Low. **Downtime:** нет.
- **Acceptance:** процедура документирована и один раз выполнена на файле без изменения содержимого; md5 сервер/repo совпадают; `.bak` предыдущей версии сохранён с меткой времени.
- **Как проверить:** `md5sum` до/после; `ls -l /root/insurance_broker/scripts/*.bak-*`; `./scripts/backup-db-telegram.sh --help`.
- **Rollback:** восстановить `.bak`.
...
- **Rollback:** `.bak` трёх файлов (доставляются одним набором).
...
- **P0-03 строго перед всеми правками скриптов**, иначе исправление осядет в репозитории, а прод продолжит nightly падать — ровно та ситуация, в которой мы уже находимся с верификацией.
...
- **D1 — Доставка скриптов на сервер (P0-03):** `scp` отдельного файла + сверка md5 с репозиторием + timestamped `.bak` на сервере перед заменой. Без ansible и без git-чекоута на проде; откат = копирование `.bak` обратно. (Вариант с git-клоном добавляет на прод-сервер сетевой доступ к репо и состояние ветки — лишняя сложность при трёх файлах.)
...
| P0 | P0-03 | Канал доставки скриптов на сервер |
