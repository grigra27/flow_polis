...
- **Удаление (2026-09-23, по прямому разрешению владельца на пункты 1, 2, 3, 4, 6 из таблицы Q5):**
  ...
  - **`restore-db.sh` — отдельная, более серьёзная находка, намеренно НЕ исправлена в этом заходе.** И `docs/BACKUP_RESTORE.md` (раздел «Database Restore», 9 шагов с `--interactive`/`--latest`/`--file`/`--list`), и `scripts/README.md` (секция `### restore-db.sh`) подробно описывают интерфейс скрипта, которого **не существует в репозитории вообще** — ни сейчас, ни, судя по всему, раньше. Реальный (и единственный) инструмент восстановления — `scripts/import-database.sh <файл>`, деструктивный, без интерактивности, официально помеченный в этом же бэклоге (D2) как «не использовать для routine-операций». Переписывать целую секцию про disaster recovery с нуля — отдельная, требующая аккуратности задача, не входившая в согласованный scope пункта 5; оставлена как открытый вопрос владельцу.
...
- **P2-06 — опись сделана, удаление выполнено полностью.** Владелец разрешил по Q5; пункты 1,2,3,4,6 удалены и проверены в тот же день. Пункт 5 (`backup-db.sh`/`backup-media.sh`) — пред-правка (`setup-backup-cron.sh` + 2 документа) сделана следом, файлы удалены и из репозитория, и с сервера. Единственное намеренно не тронутое — фиктивный `restore-db.sh`, отдельная находка вне scope.
