...
### P2-06 — Housekeeping: лишний мусор и мёртвые конфиги (опись, без удаления) — **PASS (2026-09-23) — опись готова, удаление ждёт Q5**
...
- **P2-06 — опись сделана.** Ждёт решения владельца по Q5 для фактического удаления (это уже отдельная, следующая задача — сама P2-06 определена как «инвентаризация», не «уборка»).
...
5. **Q5 — Разрешение на удаление housekeeping-мусора (P2-06). Опись готова (2026-09-23), решение за вами по каждому пункту:**

   | № | Что | Где | Безопасно удалить? |
   |---|---|---|---|
   | 1 | `scripts/ystemctl start cron` (случайный вывод `systemctl status`) | сервер | Да, без оговорок |
   | 2 | `scripts/~/insurance_broker_backups/` (старый дамп 09.12.2025) | сервер | Да, без оговорок |
   | 3 | `.env.deployment` (реликт прежнего хостинга — другой IP/домен/APP_DIR, нигде не читается) | сервер | Да, без оговорок |
   | 4 | Docker volume `insurance_broker_media` (пустой, 0 файлов, не в docker-compose) | сервер | Да (`docker volume rm` — отдельным шагом) |
   | 5 | `backup-db.sh` / `backup-media.sh` | репозиторий + сервер | **Нет — сначала** поправить `setup-backup-cron.sh` + 3 места в документации, которые всё ещё выдают их за актуальные, **потом** удалять |
   | 6 | `backup-db-telegram.sh.bak-20260919_222023` (устарел на 2 правки P0-06) | сервер | Да, без оговорок |
   | 7 | `scripts/db_backup.sql` (прод-дамп, 2.2 МБ) | **только ваша локальная машина**, в git не попадёт | Вне scope этой задачи — рекомендую удалить самостоятельно, не буду трогать локальные файлы без прямой просьбы |

   Разрешаете удаление пунктов 1–4 и 6 сейчас, и отдельно — правку + удаление по пункту 5?
...
| P2-06 | **PASS (2026-09-23)** — опись готова, 7 находок с рекомендациями, удаление ждёт Q5, см. статус P2-06 |
