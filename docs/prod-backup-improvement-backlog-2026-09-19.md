...
- **Статус:** **Реализовано, не задеплоено (2026-09-23).** Новый общий модуль `scripts/backup-retention.sh` (функция `prune_backups`, sourced обоими backup-скриптами) — файлы передаются `ls -t` (портируемо между GNU/BSD) в порядке «новые первыми»; позиции `[0, MIN_RETAINED_BACKUPS)` не удаляются никогда, независимо от возраста; за этим порогом удаляется (или, при `PRINT_ONLY=true`, только логируется) всё старше `RETENTION_DAYS`. Дефолты выставлены как конкретизация предложения Q3 (пересмотренного): `RETENTION_DAYS=30` / `MIN_RETAINED_BACKUPS=5` для DB, `RETENTION_DAYS=28` / `MIN_RETAINED_BACKUPS=4` для media (~4 недельных прогона) — обе переменные оставлены переопределяемыми через env, отдельного шага «подтверждение Q3» не проводилось, решение принято как инженерное при реализации по прямому указанию продолжить P1-05. Media: `.tar.gz` и `.empty`-маркеры собираются в одну последовательность «свежести» через `ls -t` над обоими паттернами — floor работает поперёк типов. Попутно исправлена сопутствующая правке функция подчистки `.meta`-сайдкаров: у DB-скрипта имя `.meta` реконструировалось неверно (баг существовал до этой задачи, см. `git blame`), из-за чего осиротевшие `.meta`-файлы могли копиться бессрочно; теперь и удаление пары «архив+meta», и разовая подчистка настоящих сирот (без парного архива) работают на реальных именах файлов. `PRINT_ONLY=true` также отключает `notify_cleanup_result` — dry-run не шлёт уведомлений. Валидация некорректных `RETENTION_DAYS`/`MIN_RETAINED_BACKUPS` (по образцу `MIN_BACKUP_BYTES`) — безопасный fallback с warning, не падение. Тесты — `scripts/tests/test-backup-retention.sh`, 12/12 (floor+age, floor превышает популяцию, dry-run не трогает диск и не шлёт уведомлений, подчистка сирот в реальном/dry-run режиме, media со смешанными типами, невалидные env). Полный набор бэкап-тестов — 265+12=277/277, регрессий нет (в 5 существующих suite добавлен `backup-retention.sh` в список копируемых файлов изолированного sandbox). Документация обновлена в рамках P1-06 (общий diff, см. её статус).
- **Не сделано:** пуш/деплой; живое подтверждение на боевом cron-прогоне.
...
- **Статус:** **PASS (2026-09-23).** `docs/BACKUP_RESTORE.md`: раздел «Backup Storage» переписан — новый подраздел «External copy: VK/Telegram mirror — accepted risk, no independent offsite store» покрывает все пять пунктов scope (нет offsite и не планируется + дата решения; VK-зеркало как факт единственной внешней копии; статистика 2/8 ночей `no_free_space`/`not saved`; что митигирует — P0-06/P0-09/P1-05 со ссылкой; что не митигирует — полная потеря сервера); попутно поправлены «Backup Strategy» (media — еженедельно, а не ежедневно) и таблица Environment Variables (актуальные `RETENTION_DAYS`/новые `MIN_RETAINED_BACKUPS`/`PRINT_ONLY` из P1-05) — эти неточности напрямую граничили с правкой и остались бы вводящими в заблуждение по соседству с исправленным текстом. `scripts/README.md`: перед секцией Telegram-интеграции добавлена явная врезка «Storage vs. notification» со ссылкой на новый раздел и на P1-07 (Telegram сетевой недоступен, реально доставляет VK); списки уведомлений `backup-db-telegram.sh`/`backup-media-telegram.sh` дополнены упоминанием VK. `scripts/telegram-notify.sh`: заголовочный комментарий файла заменён — явно указано «notification/mirror layer, NOT backup storage», со ссылкой на контракт P0-08 и на решение владельца. Контракт P0-08 (семантика полей) не менялся нигде — только документация и комментарии.
- **Не входит (сознательно оставлено как есть):** остальной текст `docs/BACKUP_RESTORE.md` и `scripts/README.md` вне тронутых разделов ссылается на не-Telegram-скрипты (`backup-db.sh`, несуществующий `restore-db.sh`) и на устаревшее ежедневное расписание media — это дрейф документации, предшествующий P1-05/P1-06 и не связанный с ролью offsite/mirror; отдельная задача по этой уборке в бэклог не заведена.
...
- **Статус:** **Реализовано, не задеплоено (2026-09-23).** Добавлен `_check_backup_freshness` в `apps/core/management/commands/system_health_check.py` (новый флаг `--check-backups`, включён также в `--check-all` и в дефолтный набор без флагов): читает `last_status.json` по контракту P0-08 для `database`/`media` — каталог ищет теми же переменными (`BACKUP_DB_DIR`/`BACKUP_MEDIA_DIR`/`BACKUP_BASE_DIR`, затем `/app/server_backups/<subdir>`), что и уже существующий `apps/reports/views.py` (второй источник правды по путям не заведён). Required-стадии не пересчитываются — используется уже готовое поле `result` из файла (его считает `evaluate_backup_result()` в `backup-status.sh`). Пороги свежести — DB < 26 ч, media < 8 сут (192 ч), как в исходном scope. Отсутствие каталога/файла, битый JSON, протухание и `result != "ok"` — во всех случаях `warning`, никогда `critical` (сам сайт не сломан). Юнит-тесты — `apps/core/tests/test_system_health_check_backup_freshness.py`, 8/8 (healthy/no-dir/no-file/stale/failed-result/corrupt-json/media-own-threshold/check-all-integration); полный `apps/core/tests/` — 116/116, регрессий нет. Cron-строка health-check уже зовёт `--check-all` (см. P1-10) — новая проверка подключится сама следующим деплоем, отдельной правки cron не требуется.
- **Не сделано:** пуш/деплой (жду отдельной команды — commit ≠ разрешение на push); живое подтверждение на проде (порча `last_status.json` внутри контейнера + `--notify-vk`, проверка, что сигнал доходит в VK); наблюдение за первым media-прогоном под новым кодом (не раньше понедельника 28.09 03:00 MSK).
...
### P2-01 — `PaymentSchedule.clean()`: не подменять стандартную валидацию обязательных полей — **PASS (2026-09-23)**
...
- **Rollback:** revert коммита.
- **Статус:** **PASS (2026-09-23).**
  - **Уточнение по факту (важно):** эмпирически проверено (тест на `full_clean()` до правки), что даже **до** фикса `year_number=None` уже корректно отклонялся с `ValidationError({'year_number': [...]})`, а не проходил в БД — потому что `full_clean()` вызывает `clean_fields()` (шаг 1) **до** кастомного `clean()` (шаг 2) и собирает ошибки со всех шагов в один `ValidationError`, независимо от того, что происходит внутри `clean()`. То есть баг был не в «портятся данные», а в лишнем внутреннем `ValueError`, который перехватывался, логировался как ERROR-traceback + WARNING и маскировал понятную причину шумной вторичной ошибкой — ровно как было отмечено ещё на этапе критического разбора бэклога 21.09.
  - **Follow-up по policy 526 — закрыт, ремонт не требуется:** прямой read-only запрос на проде (`SELECT … WHERE year_number IS NULL OR installment_number IS NULL`) — **0 строк**; обе колонки `NOT NULL` на уровне БД. Других мест, создающих `PaymentSchedule` в обход `save()`/`full_clean()` (`bulk_create`, прямой `QuerySet.update()`), в коде приложения не найдено. Инцидент 2026-09-14 был просто отклонённой попыткой сохранения, данных на ремонт нет.
  - **Правка:** ранний выход в `clean()` при `not self.policy_id or self.year_number is None or self.installment_number is None` — до попытки построить `Q(year_number__lt=None)`. `pk` в проверке не участвует, как и требовал scope.
  - **Тесты:** `apps/policies/tests/test_payment_schedule_clean_required_fields.py`, 5/5 — (a) `year_number=None`/`installment_number=None` → `ValidationError`, не `ValueError`; запись не сохраняется; ERROR-traceback и WARNING «Manual review recommended» больше не появляются; (b) новый `PaymentSchedule` с `pk=None` по-прежнему проверяется на последовательность дат — и при нарушении получает `ValidationError` по `due_date`, и при валидной последовательности успешно сохраняется (доказывает, что проверка не пропускается молча). Полный `apps/policies/` — 91/91 (без учёта уже известного, не связанного с этой задачей флейки-теста Hypothesis на суррогатных UTF-16 символах в `test_payment_properties.py`, воспроизводимого и без каких-либо правок).

### P2-02 — Отсутствие `CommissionRate` для insurer=17 / insurance_type=1 — **PASS (2026-09-23)**
...
- **Статус:** **PASS (2026-09-23).** По всей базе read-only проверкой найдено ровно **две** непокрытые пары (не одна, как думали изначально) — обе теперь закрыты решением владельца:
  1. **Югория (insurer=17) / КАСКО (type=1) — 5 полисов, 11 платежей, ~2.36 млн ₽.** Владелец подтвердил: реальная ошибка, договор есть; добавлена ставка **30%** (`CommissionRate id=56`, 2026-09-23) через admin. **До правки зафиксировано (снимок «до», сохранён в этой задаче):** `id=1284/pол.636/56710/0.00`, `1488,1489,1490/пол.712/330000×3/82500×3`, `2195,2196/пол.1094/232700×2/69810×2`, `2469,2470/пол.1191/129696×2/38908.80×2`, `2473,2474,2475/пол.1193/196320×3/58896×3` — все `commission_rate_id=NULL`. Обнаружен и обойдён латентный баг в `signals.py`: эвристика «правили руками» сравнивает старый `kv_rub` с тем, что дала бы формула **без** ставки (всегда 0) — для платежа 1284 (`kv_rub=0`, менеджер сознательно отказался от КВ) это неотличимо от «ещё не считали», и первое же сохранение через админку после появления ставки тихо пересчитало бы 0 → 17013.00. Владелец поправил все 11 платежей вручную через админку: для 1284 — двухшаговым сохранением (сначала ставка → пересчёт в 17013, затем то же самое сохранение с КВ, вручную возвращённым к 0 — на этом шаге `commission_rate` уже не меняется, пересчёт не срабатывает); для остальных 10 — одним сохранением каждый (их `kv_rub` был ненулевым, эвристика верно определила «редактировано вручную» и не тронула). **После правки проверено (read-only):** все 11 строк — `commission_rate_id=56`, `kv_rub` побайтово те же значения, что и «до» (0.00 и 82500.00 сохранены как есть — отказ от КВ и пониженная до 25% ставка по одной сделке подтверждены владельцем как корректные, не трогались); в `django.log` — ровно по одной строке `Auto-set commission rate 56 for payment policy=...` на первое сохранение каждого затронутого полиса, ни одной ошибки/traceback. Эти 11 платежей теперь защищены: при следующем обычном сохранении `commission_rate` не изменится → блок пересчёта `kv_rub` не выполнится.
  2. **Чулпан (insurer=15) / КАСКО (type=1) — 3 полиса, 10 платежей, ~80 тыс ₽.** Владелец подтвердил: договора нет, реальное легитимное отсутствие ставки. Фейковая ставка `0%` не заводилась — это исказило бы смысл (выглядело бы как «работаем бесплатно», а не «нет отношений»). Вместо этого понижен уровень лога: `apps/policies/signals.py` — ветка `CommissionRate.DoesNotExist` теперь пишет `logger.info(...)` вместо `logger.warning(...)`, с тем же диагностическим контекстом (policy/insurer/insurance_type), плюс комментарий о причине. После правки эти 10 платежей не подтверждены read-only проверкой на проде (`commission_rate_id=NULL`, `kv_rub=0` для всех 10) — легитимное отсутствие, шум ушёл на уровень INFO.
  - Других непокрытых пар в базе не найдено (проверено `GROUP BY insurer, insurance_type` по всем `PaymentSchedule` без `commission_rate_id`).
  - Тесты: `apps/policies/tests/test_missing_commission_rate_log_level.py` (2 теста — уровень INFO не WARNING, диагностический контекст сохранён); `apps/policies/` — 86/86 (не считая заведомо не связанного, воспроизводимого и без этой правки, флейки-теста Hypothesis на суррогатных UTF-16 символах в `test_payment_properties.py`).

### P2-03 — logrotate для cron-логов в `/root/insurance_broker/logs/` — **PASS (2026-09-23)**
...
- **Scope:** правило для `backup-db.log`, `backup-media.log`, `backup-cleanup.log`, `health-check.log`, `daily-digest.log`, `cleanup-login-attempts.log`: weekly, `rotate 8`, `compress`, `missingok`, `notifempty`, `create 0644` (владелец/группа — см. поправку к D7 ниже).
...
- **Статус:** **PASS (2026-09-23).**
  - **Поправка к D7 (важно, найдено при реализации):** исходное решение — `create 0644 10001 10001` — не работает на реальном сервере. `logrotate 3.21.0` (установлен на проде) не резолвит голый числовой UID/GID в `create`, несмотря на то, что его собственный man page («USER AND GROUP») документирует резервный переход на числовое значение при неудаче текстового поиска — воспроизведено на изолированном тестовом конфиге: `error: unknown user '10001'`. `getent passwd 10001` на хосте не находит пользователя (10001 — UID процесса **внутри** контейнера, у хоста своего пользователя с таким UID нет). **Исправление:** `create 0644` без owner/group — man page отдельно документирует, что опущенные атрибуты `create` наследуются от **оригинального** (ротируемого) файла; проверено вживую `logrotate -f`: новый файл после ротации получил владельца 10001:10001 автоматически, без указания числа явно. Результат идентичен изначальному намерению D7 («не менять картину прав»), только без нерабочего синтаксиса.
  - **Установка:** конфиг записан на сервере как источник правды из `scripts/setup-logrotate.sh:generate_logrotate_config()` (побайтово сверено). Сам скрипт доставлен **не** через `scp` — секундная ошибка процесса в разговоре: доставил файл на сервер вручную для проверки, тут же удалил, канонический путь доставки — обычный git push + деплой, как и для всех остальных скриптов (D1).
  - **Живая проверка на проде (не в изоляции):**
    1. `logrotate -d /etc/logrotate.d/insurance-broker` — 0 ошибок, `Handling 1 logs` (один stanza на все 6 путей).
    2. `logrotate -f` — реальная принудительная ротация всех 6 файлов: `backup-db.log`, `backup-media.log`, `backup-cleanup.log`, `health-check.log`, `daily-digest.log`, `cleanup-login-attempts.log` → каждый стал `<name>.log.1.gz` (сжат), рядом создан новый пустой `<name>.log` (0 байт, `0644`, владелец `10001:10001` — унаследован корректно).
    3. `django.log` (1 722 638 байт) и `security.log` (206 363 байт) — не тронуты, размеры и mtime не изменились.
    4. exit code `logrotate -f` — `0`.
  - **Пункт acceptance «следующий cron-прогон дописывает в тот же путь»** отдельно не выжидался — это гарантия семантики shell-редиректа `>> file 2>&1` (переоткрытие по имени при каждом запуске), а не результат работы logrotate; подтвердится штатно на ближайшем health-check (раз в 30 мин) без отдельного вмешательства.
  - **Тесты:** `scripts/tests/test-setup-logrotate.sh`, 16/16 — все 6 путей присутствуют, `django.log`/`security.log` не входят как цели ротации (только в пояснительном комментарии), все директивы D7 на месте, `create` не содержит захардкоженного числового owner/group, путь считается из аргумента `app_dir`, а не захардкожен, ровно один stanza. Реальную синтаксическую проверку через `logrotate -d`/`-f` тест не дублирует — `logrotate` недоступен локально (Ubuntu-специфичный пакет), это сделано вживую на проде (см. выше).

### P2-04 — DEBUG-шум в `daily_digest` — **PASS (2026-09-23)**
- **Проблема:** `daily-digest.log` на 2.4 MB состоит из `DEBUG: Processing updated policy …`; полезная часть тонется. Уточнение по факту (задача изначально описывала это как «настройка уровней логирования» — на деле это был обычный `print("DEBUG: ...")`, вообще не через `logging`, то есть уровень логов на него никак не влиял): всего в файле нашлось **22 таких `print()`**, а не один — построчная трассировка логинов, изменений полисов, платежей и форматирования сообщения, плюс 3 `print("ERROR: ...")` при неудачной отправке части дайджеста.
...
- **Rollback:** revert.
- **Статус:** **PASS (2026-09-23).** Все 22 `print("DEBUG: ...")` конвертированы в `logger.debug(...)` (модуль уже импортировал `logging` и имел `logger = logging.getLogger(__name__)`, просто не использовал его для этой трассировки). Новый флаг `--verbose` в `handle()` поднимает уровень just для этой команды через `logger.setLevel(logging.DEBUG)`, с восстановлением исходного уровня в `finally` — без этого мутация синглтон-логгера пережила бы вызов и просочилась в последующие запуски в том же процессе (важно для тестов и для гипотетического long-running процесса). В проде root logger уже настроен на INFO (`config/settings.py`), поэтому без флага `logger.debug(...)` теперь **не создаёт даже LogRecord** — тише, чем просто «не пишется в файл». 3 `print("ERROR: ...")` при неудачной отправке части дайджеста намеренно не тронуты — вне заявленного scope (это не «детальный перебор», а сообщения о реальных сбоях), решение оставить как есть, а не конвертировать в `logger.error()` (что подключило бы их к Telegram-алертингу) принято намеренно, чтобы не менять поведение уведомлений в рамках задачи про логи.
  - **Замер (тестовая БД, `--test --no-telegram --no-vk`, без активности за период):** до — десятки/сотни строк на полис при реальной активности; после — **6 строк** (`📊 Генерация...`, `Период/С/По`, 2×`⏭️ пропущено`). С реальными полисами/логинами за период будет больше, но пропорционально их числу как «итоговые» строки, а не построчная трассировка каждого объекта — укладывается в ≤~30 для типичного дня.
  - **Тесты:** `apps/core/tests/test_daily_digest_logging.py`, 4/4 — тихо по умолчанию, `--verbose` даёт DEBUG-записи, отсутствие протечки уровня в следующий вызов в том же процессе, восстановление исходного уровня логгера после выполнения. Сознательно не использовал `caplog.at_level()`/`set_level()` — они сами принудительно поднимают уровень целевого логгера, что сделало бы тест тавтологичным; вместо этого — вручную подключённый `logging.Handler`, наблюдающий только то, что реально прошло бы через настоящий уровень. Полный `apps/core/tests/` — 120/120, регрессий нет.
  - **Не проверено на реальном контейнере** (P2-04 предлагал «ручной запуск в контейнере» как способ проверки) — локальный `manage.py` уткнулся в не относящееся к задаче рассогласование схемы локальной sqlite (`no such column: auditlog_logentry.changes_text`, миграции числятся применёнными, но колонки нет — отдельная, не связанная с этой задачей проблема dev-окружения); поведение подтверждено через pytest на корректно смигрированной тестовой БД вместо этого.

### P2-05 — Классификация HTTP/CSRF/security warning'ов и настройка trusted origins — **PASS (2026-09-23)**
...
- **Статус:** **PASS (2026-09-23) — решение «игнорировать», код/настройки не менялись.**
  - **Ограничение метода:** сопоставить историю аудита (9 месяцев) с nginx-логами не удалось — они пишутся в stdout контейнера и обнуляются при каждом деплое (последний рестарт nginx — 21.09 19:20, глубже данных нет). Вместо ретроспективы проверена **текущая живая картина** за 2 суток (21–23.09): все пять паттернов из аудита по-прежнему активны (например, «no Referer» ловился сегодня дважды), так что вывод актуален, а не устаревший.
  - **Факт:** за 2 суток — 4161 запрос, 16 ответов 403. **Все 16 без исключения** — слепой `POST /` (или `GET /`) с пустым Referer (`"-"`) и одной из подписей: `Go-http-client/1.1`, `libredtail-http` (один запрос — классическая PHP-эксплойт-строка `allow_url_include`/`php://input`, атака на технологию, которой в проекте нет), голый `Mozilla/5.0` без версии, UCBrowser/Android 4.4. Ни одного 403 с признаками живой сессии (Referer-цепочка, cookies, повторные визиты) не найдено.
  - **Контроль (настоящий пользователь в том же окне):** `95.161.60.204`, YaBrowser/Mac — `Referer: .../policies/` → `/accounts/login/?next=/` → `/admin/`, ответы 200 по 16+ КБ. Форма поведения принципиально другая, чем у 403-групп.
  - **www.polis.insflow.ru (99 случаев) — разобран отдельно, самый похожий на реальную проблему пункт.** DNS резолвит на тот же IP, TLS-сертификат покрывает оба домена (`DNS:polis.insflow.ru, DNS:www.polis.insflow.ru`), `www.polis.insflow.ru` есть в `ALLOWED_HOSTS` на проде — то есть браузер реально может открыть сайт под www без предупреждений. Но сама ошибка Django (`Referer … does not match any trusted origins`) возникает только когда Referer указывает на www, а фактический Host запроса — **другой**; при относительных URL в шаблонах и отсутствии редиректа www→канонический домен органическая навигация человека внутри www на это никогда не выйдет. Ошибка requires явно подставленный чужой Referer — то есть тот же класс сканеров, что и остальные 4 паттерна. Живых подтверждений не найдено.
  - **109.68.215.223 (36 случаев)** — это собственный IP сервера (см. `~/.ssh/config`, `Host polis` → `HostName 109.68.215.223`); прямые обращения по IP — стандартное поведение интернет-сканеров, не отдельный реальный актор.
  - **Решение по каждой группе:** все пять — **игнорировать**, `CSRF_TRUSTED_ORIGINS` не заводить (ни `www`, ни IP) — это разрешило бы ровно тот трафик, который сейчас корректно отбивается, не защитив ни одного реального пользователя, которых в выборке 403 просто нет. `ALLOWED_HOSTS` не менялся.
  - **Побочная находка (не в scope, не исправлялась):** `Blocked login attempt from IP 127.0.0.1` — 13 из 36 записей за всю историю висят на localhost, остальные 23 — на настоящих внешних IP. `apps/core/ip_utils.py` (PLAN 9(b)) уже содержит осознанную защиту от подделки `X-Forwarded-For` (доверяет заголовку только когда `REMOTE_ADDR` — сам nginx), и nginx корректно прокидывает `X-Forwarded-For $proxy_add_x_forwarded_for` — то есть механизм в целом исправен, 127.0.0.1 похоже на единичные прямые обращения к Django в обход nginx (health-check/ручное тестирование), а не на системную дыру в brute-force защите. Не чинилось — вне scope P2-05 (rate limiting/анти-бот исключены явно), при желании отдельная небольшая задача на будущее.

### P2-06 — Housekeeping: лишний мусор и мёртвые конфиги (опись + согласованное удаление) — **PASS (2026-09-23) — пункты 1,2,3,4,5,6 удалены (5 — после пред-правки ссылок), 7 вне scope**
...
- **Rollback:** не применимо.
- **Статус:** **PASS (2026-09-23) — опись выполнена read-only, ни один `rm`/`docker volume rm`/правка скрипта не выполнялись.** Полная таблица — ниже, в Q5. Кратко по каждому пункту:
  1. `scripts/ystemctl start cron` (сервер, 1735 байт, 18.12.2025) — не опечатка перенаправления, а буквально вывод `systemctl status cron` (с ANSI-кодами), случайно сохранённый в файл с мусорным именем. Нигде не используется. Безопасно удалить.
  2. `scripts/~/insurance_broker_backups/` (сервер, 09.12.2025) — один старый дамп БД (105 КБ) + `.meta`, попавший туда из-за нераскрытия `~` в каком-то ручном вызове бэкап-скрипта из каталога `scripts/`. Не является текущим хранилищем бэкапов (то — отдельно, штатно). Безопасно удалить.
  3. `.env.deployment` (сервер, 05.12.2025) — **находка серьёзнее, чем в исходной формулировке**: это не «одна мёртвая переменная», а **весь файл — реликт предыдущего, ныне не существующего хостинга**: другой droplet IP (`64.227.75.233`), другой домен (`onbr.site`/`www.onbr.site`), другой `APP_DIR=/opt/insurance_broker` — ничего из этого не соответствует нынешнему серверу (`109.68.215.223`, `polis.insflow.ru`, `/root/insurance_broker`). Проверено: файл нигде не подключается и не читается — ни одним скриптом, ни в репозитории, ни на сервере. Безопасно удалить целиком.
  4. Docker volume `insurance_broker_media` (пустой, сервер) — создан `2026-09-19T13:02:27Z`, ровно в день исходного аудита; 0 файлов; не упомянут ни в одном `docker-compose*.yml`. Настоящий том — `insurance_broker_media_volume` (227 файлов). Безопасно удалить (`docker volume rm` — отдельной задачей, не в этой).
  5. `scripts/backup-db.sh` / `scripts/backup-media.sh` (репозиторий + сервер) — **не просто мусор, а действующая ловушка**: cron их не использует (проверено — в `crontab -l` нет ни одного упоминания без `-telegram`), но их всё ещё запускает `scripts/setup-backup-cron.sh`, и они упомянуты как актуальные в `docs/BACKUP_RESTORE.md`, `scripts/README.md`, `scripts/setup-telegram.sh`. Если когда-нибудь (например, при восстановлении на новом сервере) кто-то по этой документации выполнит `./scripts/setup-backup-cron.sh`, встанет **параллельный, устаревший, без статус-контракта P0-08 и без ставки на VK/Telegram** cron поверх настоящего. Простое удаление файлов **недостаточно** — сначала нужно поправить `setup-backup-cron.sh` (указать на `*-telegram.sh`) и три места в документации, и только затем удалять сами файлы (либо оставить как исторический образец с явной пометкой «не использовать», раз в `backup-db.sh:255` был взят паттерн верификации для P0-04).
  6. `scripts/backup-db-telegram.sh.bak-20260919_222023` (сервер, уже отмечен в P0-03) — проверено побайтово: старее текущего рабочего скрипта на две правки (P0-06's `MIN_BACKUP_BYTES`, `source backup-status.sh` отсутствуют) — то есть неактуален уже давно, а не просто «безвредная копия». Безопасно удалить.
  7. **Новая находка, не из исходного аудита:** `scripts/db_backup.sql` (**только на локальной машине**, 2.2 МБ, 30.12.2025) — полный дамп прод-БД лежит в рабочей копии репозитория, синхронизируемой Яндекс.Диском. В git не попадёт (файл в `.gitignore`), но это реальные производственные данные без шифрования в облачно-синхронизируемой папке на личном устройстве. Не входит в scope этой задачи (не сервер, не репозиторий), но стоит вашего внимания отдельно от Q5 — рекомендую удалить локально самостоятельно.
- **Удаление (2026-09-23, по прямому разрешению владельца на пункты 1, 2, 3, 4, 6 из таблицы Q5):**
  - Перед удалением подтверждено `ls`/`docker volume inspect` по каждой из пяти целей — совпадают с описанными в описи путями/размерами/датами.
  - Удалено на сервере: `scripts/ystemctl start cron`; `scripts/~/insurance_broker_backups/` (каталог целиком); `.env.deployment`; `scripts/backup-db-telegram.sh.bak-20260919_222023`; Docker volume `insurance_broker_media` (`docker volume rm`).
  - После удаления проверено: все пять путей/volume отсутствуют (`ls`/`docker volume inspect` → "No such file or directory" / "no such volume"); настоящий volume `insurance_broker_media_volume` не тронут (227 файлов, как и было); рабочий каталог `~/insurance_broker_backups/database/` со свежими бэкапами (21–23.09) не тронут; `scripts/` больше не содержит ни `ystemctl`, ни `.bak`, ни `~`.
  - **Пункт 5 (`backup-db.sh`/`backup-media.sh`) — пред-правка выполнена, файлы удалены (2026-09-23).** Поправлены три места, ссылавшиеся на них как на актуальные: `scripts/setup-backup-cron.sh` (проверка наличия файлов, генерируемые cron-строки, отображаемое расписание, фильтр очистки старых строк перед переустановкой — переведены на `*-telegram.sh` и на реальное расписание media/cleanup — еженедельно по понедельникам, а не ежедневно/по воскресеньям); `docs/BACKUP_RESTORE.md` (раздел Overview, все примеры команд, блок автоматизации cron, троеточие в Troubleshooting — 13 правок, плюс добавлена явная заметка об удалении старых скриптов); `scripts/README.md` (убраны отдельные секции `### backup-db.sh`/`### backup-media.sh`, переформулированы описания `backup-db-telegram.sh`/`backup-media-telegram.sh` как основных, убран шаг миграции на несуществующий уже файл). После этого файлы `scripts/backup-db.sh`/`scripts/backup-media.sh` удалены из репозитория (`git rm`) **и отдельно с сервера по SSH** — обнаружено, что деплойный `rsync` в `.github/workflows/deploy.yml` не использует `--delete`, то есть удаление файла из репозитория и пуш **не** удаляют его с сервера; без ручного вмешательства файлы остались бы там навсегда. Рабочие `*-telegram.sh` скрипты на сервере не задеты, размеры и права прежние.
  - **`restore-db.sh` — отдельная, более серьёзная находка, исправлена отдельным заходом (2026-09-23, по прямому запросу владельца).** И `docs/BACKUP_RESTORE.md` (раздел «Database Restore», 9 шагов с `--interactive`/`--latest`/`--file`/`--list`), и `scripts/README.md` (секция `### restore-db.sh`) подробно описывали интерфейс скрипта, которого **не существовал в репозитории вообще**. Читая реальный `scripts/import-database.sh` построчно, обнаружено: (1) принимает **несжатый** `.sql`, `.sql.gz` нужно предварительно `gunzip`; (2) **не спрашивает подтверждения** — дропает БД сразу на шаге 7, в отличие от заявленного вымышленным `restore-db.sh` «Ask for confirmation»; (3) не читает `.env.prod` сам — интерполяция `POSTGRES_*` в `docker-compose.prod.yml` требует `set -a; source .env.prod; set +a` перед запуском, иначе свежесозданные контейнеры получат пустые переменные; (4) реально проверяет `.env.prod` **и** `.env.prod.db` — второй файл давно не нужен по архитектуре (PLAN 11.7 консолидировал `POSTGRES_*` в `.env.prod`), но на проде физически ещё существует, поэтому проверка сегодня проходит, хотя нигде не используется; (5) авто-бэкап перед дропом кладётся в `current_db_backup_<timestamp>.sql` **в текущую рабочую директорию**, а не в `~/insurance_broker_backups/`; (6) не перезапускает celery/nginx после восстановления — это ручной шаг. `docs/BACKUP_RESTORE.md` (раздел «Database Restore» переписан целиком под реальное поведение + врезка-предупреждение наверху; поправлены шаг 5 в Disaster Recovery и раздел Restore Issues) и `scripts/README.md` (секция `restore-db.sh` удалена, у существующей документации `import-database.sh` добавлено явное предупреждение об отсутствии подтверждения) теперь описывают только то, что реально есть. Сам `scripts/import-database.sh` не менялся — документация синхронизирована с его фактическим поведением, а не наоборот.
  - **Пункт 7 (`scripts/db_backup.sql`)** — локальный файл, вне scope, оставлен на усмотрение и личное действие владельца.
  - Rollback для удалённого: не применимо (данные не уникальны — старый тестовый дамп 09.12.2025 дублирует то, что уже есть в штатных бэкапах; `.env.deployment`/`.bak`/`ystemctl`-файл не несли функциональной нагрузки; пустой volume не содержал данных).
...
Фаза C (наблюдаемость на принятой архитектуре, без offsite) — код готов, ждёт деплоя:
  P1-10 (PASS) → P1-08 (реализовано) → P1-05 (реализовано) → P1-06 (PASS)
  [P1-01, P1-02, P1-03, P1-04, P1-09 — CANCELLED решением владельца 2026-09-23]

Фаза D (прикладные и эксплуатационные, по значимости):
  P2-02 (PASS) → P2-05 (PASS) → P2-04 (PASS) → P2-01 (PASS) → P2-03 (PASS) → P2-06
```
...
- **P1-05 и P1-06 — низкий риск, без внешних зависимостей**, сделаны параллельно с P1-08 2026-09-23 (см. их статусы) — P1-06 не зависит от деплоя (документация), P1-05 ждёт деплоя вместе с P1-08.
- **P2-02 первой в фазе D — сделано.** Была единственной задачей с прямым денежным эффектом (недостающая КВ); закрыта 2026-09-23 сразу по обеим найденным парам (см. статус P2-02).
- **P2-05 — сделано.** Классификация трафика подтвердила: все пять паттернов — сканерский шум (проверено на живых nginx-логах и контрольном примере настоящего пользователя), реальных пользователей не задевает. Код/настройки не менялись, как и предполагал D8 на случай отсутствия подтверждённого влияния.
- **P2-04 — сделано.** Оказалась больше, чем описана (22 `print("DEBUG: ...")`, а не одна строка), но той же природы — чистый log-noise fix без изменения содержания дайджеста.
- **P2-01 — сделано.** Подтверждено: битых данных не было и не могло быть (`full_clean()` уже отклонял `NULL` через стандартную field-валидацию) — правка убрала только лишний внутренний `ValueError`/шумный traceback. Follow-up по policy 526 закрыт: 0 строк с `NULL`, ремонт не нужен.
- **P2-03 — сделано.** Живьём проверено на проде: `logrotate -f` реально ротировал все 6 файлов, `django.log`/`security.log` не тронуты. По пути нашлась и исправлена ошибка в исходном D7 (числовой UID в `create` не резолвится этой сборкой logrotate).
- **P2-06 — опись сделана, удаление выполнено полностью, включая находку сверх исходного scope.** Владелец разрешил по Q5; пункты 1,2,3,4,6 удалены и проверены в тот же день. Пункт 5 (`backup-db.sh`/`backup-media.sh`) — пред-правка (`setup-backup-cron.sh` + 2 документа) сделана следом, файлы удалены и из репозитория, и с сервера. Побочная находка (фиктивный `restore-db.sh`) сначала оставлена как открытый вопрос, затем по прямому запросу владельца исправлена тем же днём — документация приведена в соответствие с реальным поведением `import-database.sh`.
...
4. **Q4 — Бизнес-значение CommissionRate (P2-02). ОТВЕЧЕН (2026-09-23).** Югория/КАСКО — реальная ошибка, ставка 30% внесена владельцем. Чулпан/КАСКО — договора нет, отсутствие ставки легитимно. Подробности и проверка — статус P2-02.
5. **Q5 — Разрешение на удаление housekeeping-мусора (P2-06). ОТВЕЧЕН и исполнен (2026-09-23).**

   | № | Что | Где | Безопасно удалить? | Итог |
   |---|---|---|---|---|
   | 1 | `scripts/ystemctl start cron` (случайный вывод `systemctl status`) | сервер | Да, без оговорок | ✅ Удалено 2026-09-23 |
   | 2 | `scripts/~/insurance_broker_backups/` (старый дамп 09.12.2025) | сервер | Да, без оговорок | ✅ Удалено 2026-09-23 |
   | 3 | `.env.deployment` (реликт прежнего хостинга — другой IP/домен/APP_DIR, нигде не читается) | сервер | Да, без оговорок | ✅ Удалено 2026-09-23 |
   | 4 | Docker volume `insurance_broker_media` (пустой, 0 файлов, не в docker-compose) | сервер | Да (`docker volume rm` — отдельным шагом) | ✅ Удалено 2026-09-23 |
   | 5 | `backup-db.sh` / `backup-media.sh` | репозиторий + сервер | Да, но после пред-правки ссылок | ✅ Пред-правка + удаление выполнены 2026-09-23 |
   | 6 | `backup-db-telegram.sh.bak-20260919_222023` (устарел на 2 правки P0-06) | сервер | Да, без оговорок | ✅ Удалено 2026-09-23 |
   | 7 | `scripts/db_backup.sql` (прод-дамп, 2.2 МБ) | **только ваша локальная машина**, в git не попадёт | Вне scope этой задачи — рекомендую удалить самостоятельно, не буду трогать локальные файлы без прямой просьбы | ⏳ На усмотрение владельца |

   Владелец разрешил удаление пунктов 1–4 и 6 (2026-09-23). Выполнено, проверено (`ls`/`docker volume inspect` подтверждают отсутствие), рабочие данные (настоящий media-volume, актуальные бэкапы) не задеты. Подробности и способ проверки — статус P2-06.
...
- **D7 — Ротация логов (P2-03):** `create`, обоснование — в тексте P2-03 (cron-логи не удерживаются открытыми, подтверждено `lsof`). **Уточнено при реализации (2026-09-23):** владелец нового файла указывается через `create 0644` **без** явных owner/group (наследуются от ротируемого файла), а не `create 0644 10001 10001` — прод-`logrotate` 3.21.0 не резолвит голый числовой UID вопреки собственному man page; см. статус P2-03 для воспроизведения.
- **D8 — Канонические host'ы для CSRF/ALLOWED_HOSTS (P2-05): вопрос закрыт как ожидаемый шум сканеров (2026-09-23).** `https://polis.insflow.ru` как единственный доверенный origin; `www`-вариант не добавлен — P2-05 не нашла ни одного подтверждённого 403 у реального пользователя (все проверенные случаи — сканерский трафик); доступ по IP `109.68.215.223` доверенным не сделан (это собственный IP сервера). `CSRF_TRUSTED_ORIGINS`/`ALLOWED_HOSTS` не менялись.
...
| P1 | P1-05 | Retention локально (offsite-часть снята) | реализовано, не задеплоено |
| P1 | P1-06 | Документировать принятую архитектуру (VK = внешняя копия) | PASS |
| P1 | P1-07 | Диагностика недоступности Telegram + выбор канала | PASS |
| P1 | P1-08 | Мониторинг свежести бэкапа (dead man's switch) | реализовано, не задеплоено |
...
| P2 | P2-01 | `PaymentSchedule.clean()` и валидация обязательных полей | PASS |
| P2 | P2-02 | Отсутствие `CommissionRate` insurer=17 / type=1 | PASS |
| P2 | P2-03 | logrotate для cron-логов | PASS |
| P2 | P2-04 | DEBUG-шум в `daily_digest` | PASS |
| P2 | P2-05 | Классификация HTTP/CSRF/security warning'ов | PASS |
| P2 | P2-06 | Housekeeping-опись мусора | PASS |

Итого 25 задач: 9 × P0 (все PASS), 10 × P1 (5 CANCELLED — P1-01…04 + P1-09, 3 PASS — P1-06, P1-07, P1-10, 2 реализовано/не задеплоено — P1-05, P1-08), 6 × P2 (все 6 PASS — P2-01…P2-06). Дубликатов ID нет, нумерация в каждой группе непрерывна (P0-01…P0-09, P1-01…P1-10, P2-01…P2-06). Отменённые задачи сохраняют номер в реестре — не переиспользуется и не удаляется, чтобы решение оставалось прослеживаемым. Удаление мусора по P2-06 выполнено (2026-09-23) для пунктов 1,2,3,4,6 по разрешению владельца (Q5); пункт 5 намеренно оставлен до пред-правки `setup-backup-cron.sh`/документации.
...
| P1-08 | реализовано, не задеплоено (2026-09-23) — код + 8/8 юнит-тестов, ждёт push/деплоя и живого подтверждения на проде |
| P1-05 | реализовано, не задеплоено (2026-09-23) — `scripts/backup-retention.sh` + 12/12 тестов, ждёт push/деплоя |
| P1-06 | **PASS (2026-09-23)** — документация переписана, контракт P0-08 не менялся |
| P2-02 | **PASS (2026-09-23)** — Q4 отвечен, Югория/КАСКО и Чулпан/КАСКО закрыты, см. статус P2-02 |
| P2-05 | **PASS (2026-09-23)** — все 5 паттернов классифицированы как сканерский шум, код/настройки не менялись, см. статус P2-05 |
| P2-04 | **PASS (2026-09-23)** — 22 `print("DEBUG:")` → `logger.debug`, `--verbose`, 4/4 тестов, см. статус P2-04 |
| P2-01 | **PASS (2026-09-23)** — подтверждено: битых данных не было и не могло быть; правка убрала лишний внутренний `ValueError`; 5/5 тестов; см. статус P2-01 |
| P2-03 | **PASS (2026-09-23)** — живая проверка `logrotate -d`/`-f` на проде, поправка к D7 (числовой UID не резолвился), 16/16 тестов, см. статус P2-03 |
| P2-06 | **PASS (2026-09-23)** — опись из 7 находок; владелец разрешил удаление; пункты 1,2,3,4,6 удалены сразу, пункт 5 — после пред-правки `setup-backup-cron.sh` и двух документов; пункт 7 — локальный файл, на усмотрение владельца; см. статус P2-06 |
...
- **Обновление 2026-09-23:** владелец решил не закрывать этот пробел отдельным хранилищем — VK-зеркало осознанно принято как единственная внешняя копия на этом этапе (см. рев. 4 наверху документа). Следствия:
  ...
  - **P1-05 (реализовано, не задеплоено, 2026-09-23)** сокращена до пересмотра локального retention (единственная гарантированная страховка без offsite): `scripts/backup-retention.sh`, floor + возраст + `PRINT_ONLY`, 12/12 тестов.
  - **P1-06 (PASS, 2026-09-23)** документирует принятый риск VK-как-единственной-копии вместо мотивации будущего offsite — `docs/BACKUP_RESTORE.md`, `scripts/README.md`, `scripts/telegram-notify.sh`.
...
- Контроль свежести `last_status.json` (dead man's switch, без зависимости от drill-артефакта — P1-09 отменена), а также post-deploy cron-наблюдение (первый плановый media-прогон под `f7376b6` — понедельник 28.09 03:00 MSK) → **P1-08 (реализовано, не задеплоено, 2026-09-23)**: код `_check_backup_freshness` в `system_health_check.py` подключается через существующий `--check-all` в cron-строке (см. P1-10), 8/8 юнит-тестов; живое подтверждение на проде — после push/деплоя.
