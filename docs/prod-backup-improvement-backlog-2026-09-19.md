...
### P2-06 — Housekeeping: лишний мусор и мёртвые конфиги (опись + согласованное удаление) — **PASS (2026-09-23) — пункты 1,2,3,4,6 удалены, 5 ждёт пред-правки, 7 вне scope**
...
- **Удаление (2026-09-23, по прямому разрешению владельца на пункты 1, 2, 3, 4, 6 из таблицы Q5):**
  ...
  - **Пункт 5 (`backup-db.sh`/`backup-media.sh`) не удалялся** — как и было решено в описи, сначала нужна правка `setup-backup-cron.sh` и трёх мест в документации, это отдельный следующий шаг.
...
- **P2-06 — опись сделана и удаление выполнено.** Владелец разрешил по Q5; пункты 1,2,3,4,6 удалены и проверены в тот же день. Пункт 5 (`backup-db.sh`/`backup-media.sh`) намеренно не тронут — сначала нужна пред-правка `setup-backup-cron.sh` и документации, это отдельная будущая задача.
...
5. **Q5 — Разрешение на удаление housekeeping-мусора (P2-06). ОТВЕЧЕН и исполнен (2026-09-23).**
   ...
   | 5 | `backup-db.sh` / `backup-media.sh` | репозиторий + сервер | **Нет — сначала** поправить `setup-backup-cron.sh` + 3 места в документации, которые всё ещё выдают их за актуальные, **потом** удалять | ⏳ Не тронуто — ждёт отдельной задачи (пред-правка) |
...
| P2-06 | **PASS (2026-09-23)** — опись из 7 находок; владелец разрешил удаление, пункты 1,2,3,4,6 удалены и проверены; пункт 5 ждёт пред-правки `setup-backup-cron.sh`/документации; пункт 7 — локальный файл, на усмотрение владельца; см. статус P2-06 |
